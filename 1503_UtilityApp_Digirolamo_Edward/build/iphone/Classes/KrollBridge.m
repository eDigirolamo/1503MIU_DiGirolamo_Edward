...
NSString * k1503_UtilityApp_Digirolamo_Edward$ModuleRequireFormat = @"(function(exports){"
...
@implementation k1503_UtilityApp_Digirolamo_EdwardObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k1503_utilityapp_digirolamo_edward);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_k1503_utilityapp_digirolamo_edward gc];
}
...
{
	// create k1503_UtilityApp_Digirolamo_Edward global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k1503_UtilityApp_Digirolamo_Edward" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k1503_utilityapp_digirolamo_edward = [[k1503_UtilityApp_Digirolamo_EdwardObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k1503_utilityapp_digirolamo_edward];
	
	NSString *_k1503_utilityapp_digirolamo_edwardNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k1503_utilityapp_digirolamo_edwardNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k1503_utilityapp_digirolamo_edward valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k1503_UtilityApp_Digirolamo_Edward$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k1503_utilityapp_digirolamo_edward.kroll" 
