...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kk1503_UtilityApp_Digirolamo_EdwardUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kk1503_UtilityApp_Digirolamo_EdwardUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a k1503_UtilityApp_Digirolamo_Edward app using the node.js CLI
